 ...
 */
static void resize_rgb565(const uint16_t* src, int src_w, int src_h, uint16_t* dst, int dst_w,
                          int dst_h)
{
    for (int y = 0; y < dst_h; y++)
    {
 ...
 */
static void draw_rect_rgb565(uint16_t* buf, int buf_w, int buf_h, int x0, int y0, int x1, int y1,
                             uint16_t color)
{
    /* Clamp */
    if (x0 < 0)
...
extern "C" void app_main(void)
{
    ...
    if (!rgb_buf)
    {
        ESP_LOGE(TAG, "Failed to allocate RGB buffer in PSRAM (%u bytes)", (unsigned)rgb_buf_size);
        return;
    }
    ...
    while (true)
    {
        ...
        resize_rgb565_to_rgb888(frame, CAM_WIDTH, CAM_HEIGHT, rgb_buf, MODEL_W, MODEL_H);
        ...
        bsp_display_lock(0);
        {
            ...
            char status_txt[48];
            snprintf(status_txt, sizeof(status_txt), "%s | %d pers | %d%%", state, people_count,
                     confidence);
