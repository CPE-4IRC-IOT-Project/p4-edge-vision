...
esp_err_t bsp_voltage_init(void)
{
    ...
    /* ESP32-P4 does not support ADC calibration schemes */
    /* Calibration is disabled - raw values will be used */
    bsp_adc_cali_handle = NULL;
    return ESP_OK;
}
...
int bsp_voltage_battery_get(void)
{
    ...
    /* ESP32-P4 does not support ADC calibration */
    /* Use approximate conversion: ADC 12-bit with 3.3V reference and 12dB attenuation */
    /* Approximate formula: voltage_mV = adc_raw * 3300 / 4095 */
    if (bsp_adc_cali_handle != NULL) {
